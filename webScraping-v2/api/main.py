...
app = FastAPI(
    title="Motor de Búsqueda Visual RAG - Sala 3 API (webScraping-v2)",
...
def startup():
    ...
    try:
        cargar_indice()
        print("[API webScraping-v2] Índice de embeddings y dataset cargado correctamente.")
    except Exception as e:
        print(f"[API webScraping-v2] AVISO al cargar índice: {e}")

    try:
        get_model()
        print(f"[API webScraping-v2] Modelo CLIP '{MODEL_NAME}' cargado correctamente.")
    except Exception as e:
        print(f"[API webScraping-v2] ERROR al cargar el modelo CLIP: {e}")
